...
const CSV_FILE_PATH = path.join(process.cwd(), "public/dumpsNoPin/dumps.csv");
...
export async function GET() {
  try {
    ...
    console.log("📊 data from dumps no Pin", formattedData.length);
...
export async function POST(req) {
  try {
    ...
    const binListData = Papa.parse(binListContent, {
      header: true,
      skipEmptyLines: true,
    });
    ...
    try {
      ...
      existingDumps = Papa.parse(existingContent, {
        header: true,
        skipEmptyLines: true,
      }).data;
    } catch (err) {
    ...
    NewDumps.fieldsData.forEach((row, rowIndex) => {  // Removed slice(0, 5)
      ...
      [row].forEach((value, valueIndex) => {
        if (
          ...
          const matchedBin = binListData.data.find(
            (bin) => bin.BIN === sixDigits
          );
    ...
    return NextResponse.json(
      { message: "Data appended to dumps.csv successfully" },
      { status: 200 }
    );
  } catch (error) {
...
export async function PUT(req) {
  try {
    ...
    if (!bin || price === undefined) {
      return NextResponse.json(
        { error: "BIN and price are required" },
        { status: 400 }
      );
    }
    ...
    // Parse CSV content
    let rawData = Papa.parse(fileContent, {
      header: true,
      skipEmptyLines: true,
    }).data;
